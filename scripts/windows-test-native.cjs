...
function testAudioCommonIdentity(architecture = process.arch) {
  const executable = path.join(
    PROJECT_ROOT,
    "native", "windows", "build", architecture, "Release", "cpv-windows-audio-common-test.exe",
  );
  ...
  if (result.status !== 0) {
    throw new Error(result.stderr?.trim() || result.stdout?.trim() ||
      `VB-CABLE identity contract test exited with ${String(result.status)}`);
  }
}
