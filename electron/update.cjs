...
const REPOSITORY = "miuuyy/ChatGPT-Persona-Voice";
const RELEASE_API_URL = `https://api.github.com/repos/${REPOSITORY}/releases/latest`;
const USER_AGENT = "codex-persona-voice-updater";
const CHECKSUM_ASSET = "SHA256SUMS";
const CHECKSUM_SIGNATURE_ASSET = "SHA256SUMS.sig";
const UPDATE_SIGNING_PUBLIC_KEY = fs.readFileSync(
  path.join(__dirname, "update-signing-public.pem"),
  "utf8",
);
...
function verifyChecksumSignature(contents, encodedSignature, publicKey = UPDATE_SIGNING_PUBLIC_KEY) {
  const value = String(encodedSignature || "").trim();
  if (!/^[A-Za-z0-9+/]{86}==$/.test(value)) {
    throw new Error(`${CHECKSUM_SIGNATURE_ASSET} is not a canonical Ed25519 signature`);
  }
  ...
  if (signature.length !== 64 || !crypto.verify(
    ...
    throw new Error(`${CHECKSUM_ASSET} release signature verification failed`);
  }
  return true;
}
...
function request(url, redirects = 0) {
  return new Promise((resolve, reject) => {
    ...
    requestHandle.setTimeout(60_000, () => requestHandle.destroy(new Error("Update request timed out")));
...
function findMacApplication(root) {
  ...
  const executable = path.join(application, "Contents", "MacOS", "Codex Persona Voice");
...
function buildJob({
  ...
  if (platform === "darwin") {
    if (!atomicSwapExecutable || !path.isAbsolute(atomicSwapExecutable)) {
      throw new Error("The packaged macOS atomic updater helper is unavailable");
    }
    return {
      version,
      platform,
  ...
  if (platform === "linux") {
    const target = process.env.APPIMAGE?.trim();
    if (!target || !path.isAbsolute(target) || !fs.existsSync(target) ||
        !fs.statSync(target).isFile() || fs.lstatSync(target).isSymbolicLink() ||
        !target.endsWith(".AppImage")) {
      throw new Error("The running Linux AppImage path is unavailable");
    }
    return {
      version,
      platform,
...
function createUpdateController({
  ...
  const runtimeAvailable = typeof runtimeExecutable === "string" && path.isAbsolute(runtimeExecutable);
  const swapAvailable = platform !== "darwin" ||
    (typeof atomicSwapExecutable === "string" && path.isAbsolute(atomicSwapExecutable));
  let state = packaged && supportedAsset && runtimeAvailable && swapAvailable
    ? { status: "idle" }
    : { status: "disabled" };
  let checked = false;
  let pending = null;
  ...
  async function checkOnce() {
    ...
    try {
      ...
      const checksums = assets.find((item) => item?.name === CHECKSUM_ASSET);
      const signature = assets.find((item) => item?.name === CHECKSUM_SIGNATURE_ASSET);
      if (!asset?.browser_download_url || !checksums?.browser_download_url ||
          !signature?.browser_download_url) {
        throw new Error(
          `Release v${version} is missing ${assetName}, ${CHECKSUM_ASSET}, or ${CHECKSUM_SIGNATURE_ASSET}`,
        );
      }
      candidate = {
        version,
        ...
        checksumsUrl: validateReleaseAssetUrl(checksums.browser_download_url, version, CHECKSUM_ASSET),
        signatureUrl: validateReleaseAssetUrl(
          signature.browser_download_url,
          version,
  ...
  async function beginInstall() {
    ...
    pending = (async () => {
      ...
      const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "codex-persona-voice-update-"));
      try {
        const [checksums, signature] = await Promise.all([
          deps.downloadText(available.checksumsUrl),
          deps.downloadText(available.signatureUrl, 4 * 1024),
        ]);
        deps.verifyChecksumSignature(checksums, signature);
