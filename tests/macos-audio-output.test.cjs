...
const { NativeFrameParser, encodeAudioFrame, encodeFrame } = require("../electron/native-protocol.cjs");
...
test("macOS output writes framed PCM only after format-matched readiness", async () => {
...
test("macOS output rejects a frame that changes prepared format", async () => {
  const child = fakeChild();
  const output = new MacAudioOutput({
    helperPath: "/helpers/output",
    platform: "darwin",
...
test("macOS output binds a requested recording device by stable UID", async () => {
  ...
  const session = await preparing;
  assert.deepEqual(probes, [["--self-test", "--device-uid", "BlackHole2ch_UID"]]);
  assert.deepEqual(spawns[0].args, [
    "--sample-rate", "24000", "--channels", "1", "--device-uid", "BlackHole2ch_UID",
  ]);
  await session.close();
});

test("an already-faulted output process is still a successfully quiesced session", async () => {
  const child = fakeChild();
  const errors = [];
  const output = new MacAudioOutput({
    helperPath: "/helpers/output",
...
test("output close retries termination proof after a failed attempt", async () => {
  const child = fakeChild();
...
test("failed readiness cleanup returns a retained output session for termination retry", async () => {
  const child = fakeChild();
  child.stdin.removeAllListeners("finish");
  let terminationAttempts = 0;
  const output = new MacAudioOutput({
    ...
    spawnProcess: () => child,
    waitForChildExit: async () => { throw new Error("not exited"); },
    terminateProcess: async () => {
      terminationAttempts += 1;
      if (terminationAttempts === 1) throw new Error("termination not proven");
      child.exitCode = 0;
...
test("a post-ready protocol violation faults the output session", async () => {
  const child = fakeChild();
  const errors = [];
  const output = new MacAudioOutput({
    helperPath: "/helpers/output",
