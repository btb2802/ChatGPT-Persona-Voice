...
function statusFrame(state) {
  return encodeFrame("status", Buffer.from(JSON.stringify({
    type: "status",
    state,
    reason: state === "engaged" ? "voice_session_active" : "voice_session_ended",
...
test("macOS route probes helper and selected process without muting", async () => {
  ...
  assert.equal((await route.probe({ sourceId: null, sourceName: null })).ready, true);
});

test("concurrent source and suppression probes share one native self-test", async () => {
  let probeCalls = 0;
  let releaseProbe;
  const route = new MacProcessRoute({
    helperPath: "/helpers/capture",
...
test("pre-ready native initialization failures with explicit route truth do not invent suppression", async () => {
  ...
  child.stdout.emit("data", encodeFrame("error", Buffer.from(JSON.stringify({
    ...
    message: "No active Core Audio process matches the selected application.",
...
test("capture queue errors preserve suppression until guard close", async () => {
  ...
  const guard = await acquiring;
  const errors = [];
  route.open(settings, () => {}, (error) => errors.push(error));
  child.stdout.emit("data", statusFrame("engaged"));
  child.stdout.emit("data", encodeFrame("error", Buffer.from(JSON.stringify({
    type: "error",
...
test("route liveness remains subscribed after the PCM stream closes", async () => {
  ...
  child.stdout.emit("data", readyFrame());
  const guard = await acquiring;
  const source = route.open(settings, () => {}, () => {});
  child.stdout.emit("data", statusFrame("engaged"));
  await source.close();
  child.stdout.emit("data", statusFrame("armed"));
  assert.equal(statuses.at(-1).state, "armed");
...
test("route acquisition cancellation terminates the armed helper without waiting for timeout", async () => {
  ...
  const acquiring = route.acquire(settings, () => {}, () => {}, { signal: controller.signal });
  await nextTurn();
...
test("a child-process error during release cannot claim that engaged suppression was restored", async () => {
  const child = fakeChild();
  let attempts = 0;
  const { route } = fixture(child);
  route.terminateProcess = async () => {
...
test("an active child-process error remains uncertain until native lifecycle proof arrives", async () => {
  const { route, child } = fixture();
  const routeErrors = [];
  const statuses = [];
  const acquiring = route.acquire(
...
test("an armed child-process error is uncertain until clean helper release", async () => {
  const { route, child } = fixture();
  const routeErrors = [];
  const acquiring = route.acquire(settings, (error) => routeErrors.push(error), () => {});
  await nextTurn();
...
test("an active child-process error followed by exit remains permanently unproven", async () => {
  const { route, child } = fixture();
  const acquiring = route.acquire(settings, () => {}, () => {});
  await nextTurn();
  child.stdout.emit("data", readyFrame());
  ...
  child.emit("exit", 1, null);
  assert.equal(guard.originalSuppressed, true);
  await assert.rejects(() => guard.close(), /without proving route restoration/);
});

test("explicit native restoration proof clears earlier child-process uncertainty", async () => {
  const { route, child } = fixture();
  const routeErrors = [];
  const acquiring = route.acquire(settings, (error) => routeErrors.push(error), () => {});
  await nextTurn();
...
test("a capture stream fault between route readiness and source open is retained", async () => {
  ...
  assert.throws(() => route.open(settings, () => {}, () => {}), /queue fault before open/);
  assert.equal(guard.originalSuppressed, true);
  await guard.close();
});
...
test("native held-suppression truth wins before the engaged status is delivered", async () => {
  ...
  assert.equal(guard.originalSuppressed, true);
  assert.equal(guard.restorationUnproven, true);
  assert.throws(() => route.open(settings, () => {}, () => {}), /queue fault after native mute/);
...
test("coalesced readiness and proven route loss reject acquisition through owned cleanup", async () => {
  const child = fakeChild();
  const { route } = fixture(child);
  let terminated = 0;
  route.terminateProcess = async () => {
  ...
  child.stdout.emit("data", Buffer.concat([
    readyFrame(),
    encodeFrame("error", Buffer.from(JSON.stringify({
      type: "error",
      code: "source_process_exited",
...
test("coalesced readiness and restoration failure return a truthful suppression guard", async () => {
  const child = fakeChild();
  const { route } = fixture(child);
  route.terminateProcess = async () => {
    child.exitCode = 1;
  ...
  assert.deepEqual(routeErrors, []);
  assert.deepEqual(statuses, []);
  await assert.rejects(() => failure.suppressionSession.close(), /restore failed during acquisition/);
});

test("coalesced malformed route truth and failed cleanup remain conservatively uncertain", async () => {
  const child = fakeChild();
  const { route } = fixture(child);
  let attempts = 0;
  route.terminateProcess = async () => {
